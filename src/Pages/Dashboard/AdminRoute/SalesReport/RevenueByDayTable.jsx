import {
  Table,
  TableBody,
  TableCaption,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table";
import { format } from "date-fns";

const RevenueByDayTable = ({ sortedRevenueData }) => {
  return (
    <Table className="p-6">
      <TableCaption>A List Of Revenue By Day</TableCaption>
      <TableHeader>
        <TableRow className="bg-base-200 hover:bg-base-200">
          <TableHead className="px-6">Date</TableHead>
          <TableHead className="px-6">Items Sold</TableHead>
          <TableHead className="px-6">Revenue</TableHead>
          <TableHead className="px-6">Avg. Item Value</TableHead>
          <TableHead className="px-6">Estimated Tax (10%)</TableHead>
          <TableHead className="px-6">Net Revenue</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {sortedRevenueData?.length > 0 ? (
          sortedRevenueData.reverse().map((day) => {
            const avgItemValue =
              day.totalQty > 0 ? day.totalRevenue / day.totalQty : 0;
            const tax = day.totalRevenue * 0.1;
            const netRevenue = day.totalRevenue - tax;
            return (
              <TableRow key={day._id}>
                <TableCell className="px-6">
                  {day._id
                    ? format(new Date(day._id), "MMM dd, yyyy")
                    : "Invalid Date"}
                </TableCell>
                <TableCell className="px-6">
                  {day.totalQty || 0} items
                </TableCell>
                <TableCell className="px-6">
                  ৳{" "}
                  {(day.totalRevenue || 0).toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
                </TableCell>
                <TableCell className="px-6">
                  ৳{" "}
                  {avgItemValue.toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
                </TableCell>
                <TableCell className="px-6">
                  ৳{" "}
                  {tax.toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
                </TableCell>
                <TableCell className="px-6">
                  ৳{" "}
                  {netRevenue.toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
                </TableCell>
              </TableRow>
            );
          })
        ) : (
          <TableRow>
            <TableCell colSpan={6} className="px-6 text-center">
              No revenue data available.
            </TableCell>
          </TableRow>
        )}
      </TableBody>
    </Table>
  );
};

export default RevenueByDayTable;
